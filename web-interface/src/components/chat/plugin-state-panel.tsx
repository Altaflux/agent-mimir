...
export function PluginStatePanel({
  ...
  const duplicatePluginLabels = useMemo(() => {
    const counts = new Map<string, number>();
    for (const state of states) {
      const label = pluginStateLabel(state);
  ...
  const formatStateLabel = (state: PluginStateSummary) => {
    const label = pluginStateLabel(state);
    return duplicatePluginLabels.has(label)
      ? `${label} ${state.pluginInstanceId.slice(0, 8)}`
      : label;
  };

  const formatStateSubtitle = (state: PluginStateSummary) => {
    const parts = [state.agentName];
    if (duplicatePluginLabels.has(pluginStateLabel(state))) {
    ...
    for (const state of states) {
      ...
      fetch(
        ...
        .then(async (response) => {
          ...
          if (!response.ok) {
            throw new Error(
              payload.error?.message ?? "Unable to load plugin state.",
            );
          }
        ...
        .then((detail) => {
          ...
          setErrorsByInstance((current) => {
            const { [detail.pluginInstanceId]: _removed, ...rest } = current;
            return rest;
          });
        })
        .catch((error) => {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
        {!sessionId ? (
          ...
          <div className="flex min-h-full flex-col">
            <div className="mb-3 flex items-start gap-2 rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-3 py-2">
              <button
                ...
                onClick={() => setExpandedPluginInstanceId(null)}
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-semibold text-foreground">
                  {pluginStateLabel(expandedState)}
                </p>
                <p className="text-xs text-muted-foreground">
          ...
          <div className="space-y-3">
            {states.map((state) => {
              const detail = detailsByInstance[state.pluginInstanceId];
              const error = errorsByInstance[state.pluginInstanceId];
              const isLoading = loadingByInstance[state.pluginInstanceId];
                ...
                >
                  <div className="mb-2 flex items-start gap-2">
                    ...
                    <button
                      ...
                      title={`Expand ${pluginStateLabel(state)} state`}
                    >
...
function pluginStateLabel(state: PluginStateSummary): string {
  return state.pluginPrefix
    ? `${state.pluginPrefix} / ${state.pluginId}`
    : state.pluginId;
}
