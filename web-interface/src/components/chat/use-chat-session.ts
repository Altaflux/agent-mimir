...
export function useChatSession() {
  ...
  const upsertPluginStateSummary = useCallback(
    (event: Extract<SessionEvent, { type: "plugin_state" }>) => {
      setPluginStatesBySession((current) => {
        ...
        const nextState: PluginStateSummary = {
          ...
          pluginPrefix: event.pluginPrefix,
          pluginNamespace: event.pluginNamespace,
        ...
        const next = [
          ...existing.filter(
            (state) => state.pluginInstanceId !== event.pluginInstanceId,
          ),
          nextState,
        ];
        next.sort((left, right) => {
          ...
          if (pluginNamespaceComparison !== 0) return pluginNamespaceComparison;
      ...
      setEventsBySession((current) => {
        ...
        if (event.type === "agent_response_chunk") {
          if (
            existing.some(
              (e) =>
                e.type === "agent_response" && e.messageId === event.messageId,
            )
          ) {
            return current;
          }
        ...
        if (event.type === "tool_response") {
          ...
          const filtered = msgId
            ? existing.filter(
                (e) =>
                  !(e.type === "agent_response_chunk" && e.messageId === msgId),
              )
            : existing;
          if (filtered.some((e) => e.id === event.id)) return current;
          return {
  ...
  const refreshBootstrap = useCallback(async () => {
    ...
    if (!response.ok)
      throw new Error(
        apiErrorMessage(payload, "Unable to load bootstrap configuration."),
      );
  ...
  const refreshPluginStates = useCallback(
    async (sessionId: string) => {
      ...
      if (!response.ok) {
        if (
          await recoverFromSessionNotFoundResponse(sessionId, response, payload)
        ) {
          return [];
        }
        throw new Error(
    ...
    try {
      ...
      if (!response.ok) {
        if (
          ...
          return;
        throw new Error(
          apiErrorMessage(payload, "Failed to process pending notifications."),
        );
      }
      ...
      try {
        ...
        if (!response.ok) {
          if (
            ...
            return;
          throw new Error(
            apiErrorMessage(payload, "Failed to set continuous mode."),
          );
        }
  ...
  const resetSession = useCallback(async () => {
    ...
    setErrorMessage(null);
    try {
      const response = await fetch(`/api/sessions/${activeSessionId}/reset`, {
        method: "POST",
      });
      const payload = await response.json();
      if (!response.ok) {
