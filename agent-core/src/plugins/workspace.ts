...
export class WorkspanceManager {
  ...
  ): Promise<InputAgentMessage["sharedFiles"]> {
    if (responseAttributes["workspaceFilesToShare"]) {
      const files = await Promise.all(
        (JSON.parse(responseAttributes["workspaceFilesToShare"]) || []).map(
          async (file: string) => ({
            fileName: file,
            url: (await this.workspace.getUrlForFile(file))!,
          }),
        ),
      );
  ...
  async additionalMessageContent(
    nextMessage: InputAgentMessage,
  ): Promise<ComplexMessageContent[]> {
    if (nextMessage.sharedFiles && nextMessage.sharedFiles.length > 0) {
      const filesToSendMessage = nextMessage.sharedFiles
...
class WorkspacePlugin extends AgentPlugin {
  ...
  async getSystemMessages(): Promise<AgentSystemMessage> {
    ...
    const message =
      files.length > 0
        ? `You have the following files in your workspace: ${files.join(", ")}`
    ...
    return [
      {
        ...
        description:
          "The list of files from your workspace you want to send back to the user. Respond back files you want to send back or the user has requested.",
