...
describe("Plugin runtime context", () => {
  it("provides a noop runtime for standalone agent creation", async () => {
    ...
    expect(notification.pluginId).toBe("unitTest");
    expect(notification.pluginNamespace).toBe("unitTest");
  ...
  it("builds plugin contexts with plugin-specific runtime contexts", () => {
    ...
    const context = createPluginContext(
      workspace,
      pluginRuntime.bindPlugin(pluginIdentity("capturing")).runtime,
    );
