...
export function commandContentToBaseMessage(commandContent: {
  type: string;
  content: ComplexMessageContent[];
}): BaseMessage {
  const id = v4();
  const content = complexResponseToLangchainMessageContent(
...
export function lCmessageContentToContent(
  ...
  return (content as ContentBlock[])
    .map((c) => {
      ...
      if (c.type === "image") {
        ...
        const imageUrl =
          typeof imgContent.data === "string" ? imgContent.data : undefined!;
  ...
  return messages.reduce(
    (prev, next) => {
      ...
      return new SystemMessage({
        contentBlocks: [...prevContent, ...nextContent],
      });
    },
...
export function getHumanMessageSharedFiles(
...
export function readRuntimeInput(
  ...
  if (
    maybeInput.type === "user_message" &&
    isInputAgentMessage(maybeInput.message)
  ) {
    return {
      type: "user_message",
  ...
  if (
    maybeInput.type !== "plugin_notification" ||
    !maybeInput.notification ||
    typeof maybeInput.notification !== "object"
  ) {
    return undefined;
  }
  ...
  if (
    ...
    typeof notification.pluginId !== "string" ||
    typeof notification.pluginNamespace !== "string" ||
  ...
  return {
    ...
    notification: {
      ...
      pluginPrefix:
        typeof notification.pluginPrefix === "string"
          ? notification.pluginPrefix
          : undefined,
...
export function runtimeInputAdditionalKwargs(
  message: HumanMessage,
...
function isInputAgentMessage(message: unknown): message is InputAgentMessage {
  ...
  return (
    maybeMessage.sharedFiles === undefined ||
    Array.isArray(maybeMessage.sharedFiles)
  );
}

export function humanMessageToInputAgentMessage(
  message: HumanMessage,
): InputAgentMessage {
  return {
...
export function toolMessageToInputAgentMessage(
  message: ToolMessage,
): InputAgentMessage {
  return {
    content: lCmessageContentToContent(message.contentBlocks),
