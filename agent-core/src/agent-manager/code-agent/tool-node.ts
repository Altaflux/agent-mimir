...
import { ToolResponse } from "../../tools/index.js";
...
export const pythonToolNodeFunction = (
  ...
  return async (
    ...
    if (
      ...
      !(message.tool_calls ?? []).find((t) => t.name === "CODE_EXECUTION")
    ) {
      throw new Error(
        "ToolNode only accepts AIMessages as input with CODE_EXECUTION tool call.",
      );
    }

    const toolCall = (message.tool_calls ?? []).find(
      (t) => t.name === "CODE_EXECUTION",
    )!;
    ...
    const result = await executor.execute(
      ...
      libraries,
      (wsUrl, plugins) => {
        toolHandler(wsUrl, plugins, toolResponses);
      },
    );
...
export function toolHandler(
  url: string,
  plugins: RuntimePluginToolEntry[],
  toolResponses: Map<string, ToolOutput>,
) {
  ...
  ws.on("open", function open(dc: any, f: any) {
    ws.on("message", async function (data: any) {
      ...
      const entry = findRuntimeToolEntry(plugins, parsedData.request.method);

      let actualOutput;
      let error = false;
      try {
        if (!entry) {
          throw new Error(`Tool "${parsedData.request.method}" not found.`);
        }
        ...
        const output = await entry.tool.invoke(
          parsedData.request.arguments,
          toolContext,
        );

        actualOutput = output as any;
        if (entry.tool.outSchema) {
          actualOutput = JSON.parse(
            extractAllTextFromComplexResponse(
              output as ComplexMessageContent[],
            ),
          );
        } else {
