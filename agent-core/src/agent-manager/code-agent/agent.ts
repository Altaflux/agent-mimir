...
import {
    ...
    extractTextContent,
    extractTextContentFromComplexMessageContent,
...
import {
    ...
    langChainToolMessageToMimirHumanMessage,
        ...
        const callLLMNode: GraphNode<typeof AgentState> = async (state) => {
            ...
            const responseFormatSystemMessage = [
                ...
                } satisfies ComplexMessageContent,
                {
                    type: "text" as const,
                    text:
                        functionPrompt(codeExecutor.workspaceFullPath()) +
                        "\n" +
                        getFunctionsPrompt(
                            codeExecutor.availableDependencies,
                            allTools,
                        ),
                } satisfies ComplexMessageContent,
            ];
            ...
            if (
                ...
                if (ToolMessage.isInstance(lastMessage)) {
                    const inputMessage =
                        toolMessageToInputAgentMessage(lastMessage);
                    const { displayMessage, persistentMessage } =
                        await pluginContextProvider.additionalMessageContent(
                            inputMessage,
                        );
            ...
            const reformattedAiMessage = new AIMessage({
                ...
                content: complexResponseToLangchainMessageContent(
                    userMessage.tagFound ? userMessage.result : [],
                ),
                additional_kwargs: {
                    original_ai_content:
                        complexResponseToLangchainMessageContent(
