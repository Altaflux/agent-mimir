...
export class DockerPythonExecutor implements CodeToolExecutor {
  ...
  workspaceFullPath(): string {
    return (
      this.config.containerWorkspacePath ?? DEFAULT_CONTAINER_WORKSPACE_PATH
    );
  }
  ...
  async execute(
    ...
    if (!this.tempDir) {
      this.tempDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "mimir-python-code-docker-"),
      );
    ...
    const temporaryWorkspacePath = this.config.workspace
      ? undefined
      : await fs.mkdtemp(
          path.join(os.tmpdir(), "mimir-python-code-docker-ws-"),
        );
    const hostWorkspacePath =
      this.config.workspace?.workingDirectory ?? temporaryWorkspacePath!;
    ...
    const containerWorkspacePath =
      this.config.containerWorkspacePath ?? DEFAULT_CONTAINER_WORKSPACE_PATH;
    const containerRuntimePath =
      this.config.containerRuntimePath ?? DEFAULT_CONTAINER_RUNTIME_PATH;
    ...
    const containerActivatePath = path.posix.join(
      containerVenvPath,
      "bin/activate",
    );

    const createDockerRunArgs = (
      command: string,
    ...
    try {
      ...
      if (librariesToInstall.length > 0) {
        ...
        for (const libraryName of librariesToInstall) {
          ...
          const installResult = await executeCommand(
            dockerBinary,
            createDockerRunArgs(installCommand),
            {
        ...
        if (libraryInstallationResult.exitCode !== 0) {
          this.logWarn(
            `Failed to install libraries in docker:\n${libraryInstallationResult.output}`,
          );
        }
      }
      ...
      const result = await executeCommandAndTrigger(
        ...
        (data: string) => {
          if (data.includes("INITIALIZED SERVER") && !toolInitExecuted) {
            this.logInfo(
              "Python websocket server initialized. Tool bridge callback triggered.",
            );
      ...
      this.logInfo(
        `Script execution finished with exit code: ${result.exitCode}`,
      );
      ...
      if (temporaryWorkspacePath) {
        try {
          ...
          this.logInfo(
            `Removed temporary workspace directory: ${temporaryWorkspacePath}`,
          );
        } catch (error) {
