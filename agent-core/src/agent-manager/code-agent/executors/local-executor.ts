...
export class LocalPythonExecutor implements CodeToolExecutor {
  ...
  async execute(
    ...
    code: string,
    libraries: string[],
    toolInitCallback: (url: string, plugins: RuntimePluginToolEntry[]) => void,
  ): Promise<string> {
    if (!this.tempDir) {
      this.tempDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "mimir-python-code"),
      );
    }

    const temporaryWorkspacePath = (
      await fs.mkdtemp(path.join(os.tmpdir(), "mimir-python-code-ws"))
    ).replace(/\\/g, "\\\\");
    let localWorkspaceUrl = temporaryWorkspacePath;
    if (this.config.workspace) {
      localWorkspaceUrl = this.config.workspace.workingDirectory;
    }
    const tools = plugins.map((p) => p.tools).flatMap((t) => t);
    ...
    try {
      if (!this.initialized) {
        console.debug(
          `Creating python virtual environment in ${this.tempDir} ...`,
        );
        const pyenv = await executeShellCommand(
          `cd ${this.tempDir} && python -m venv .`,
        );
        if (pyenv.exitCode !== 0) {
          throw new Error(
      ...
      const librariesToInstall = externalDependencies.filter(
        (lib) => !this._installedLibraries.has(lib),
      );
      ...
      if (librariesToInstall.length > 0) {
        ...
        for (const libraryName of librariesToInstall) {
          ...
          const installationResult = await executeShellCommand(
            `cd ${path.join(this.tempDir, scriptsDir)} && ${activeScriptCall} && python -m pip install ${libraryName}`,
          );
      ...
      const result = await executeShellCommandAndTrigger(
        `cd ${path.join(this.tempDir, scriptsDir)} && ${activeScriptCall} && cd ${localWorkspaceUrl} && python ${scriptPath}`,
        (data: string) => {
          if (data.includes("INITIALIZED SERVER") && !toolInitExecuted) {
            toolInitCallback(wsUrl, plugins);
...
async function executeShellCommandAndTrigger(
  command: string,
