...
import {
  AiResponseMessage,
  NextMessagePluginNotification,
  NextMessageToolResponse,
  NextMessageUser,
...
export function langChainToolMessageToMimirToolMessage(
  message: ToolMessage,
): NextMessageToolResponse {
  return {
    type: "TOOL_RESPONSE",
...
export function langChainHumanMessageToMimirHumanMessage(
  message: HumanMessage,
): NextMessageUser | NextMessagePluginNotification {
  const base = {
  ...
  if (runtimeInput?.type === "plugin_notification") {
    return {
      ...
      pluginId: runtimeInput.notification.pluginId,
      pluginPrefix: runtimeInput.notification.pluginPrefix,
      pluginNamespace: runtimeInput.notification.pluginNamespace,
...
export function aiMessageToMimirAiMessage(
  aiMessage: AIMessage,
  files: AiResponseMessage["sharedFiles"],
  mapper: ResponseFieldMapper,
): AiResponseMessage {
  const userContent = mapper.getUserMessage(
    lCmessageContentToContent(aiMessage.contentBlocks),
  );
