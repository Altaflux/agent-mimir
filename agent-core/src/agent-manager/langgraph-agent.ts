...
export class LanggraphAgent implements Agent {
        ...
        if (state.next.length > 0 && state.next[0] === "human_review_node") {
            ...
            if (args.input) {
                ...
                graphInput = new Command({
                    resume: {
                        type: "response",
                        args: extractAllTextFromComplexResponse(
                            message.content,
                        ),
            ...
            graphInput = materializedInput
                ? {
                    ...
                    id: v4(),
                              ...
                              additional_kwargs:
                                  materializedInput.additionalKwargs,
                          }),
                      ],
    ...
    async *executeGraph(
        ...
        const getMessageChunkToStream = async (
            ...
            const markerIndex =
                streamMarkerBuffer.indexOf(USER_RESPONSE_MARKER);
            if (markerIndex === -1) {
                return { text: "", responseAttributes: responseAttributes };
            }
        ...
        while (true) {
            ...
            if (
                ...
                return {
                    ...
                    type: "toolRequest",
                    output: {
                        content: lCmessageContentToContent(
                            lastMessage.contentBlocks,
                        ),
                        id: lastMessage.id ?? "",
                        toolCalls:
                            materializeInterruptedToolCalls(interruptVal),
...
function materializeInterruptedToolCalls(
    ...
    if (Array.isArray(args)) {
        const toolCalls = args
            ...
            .filter(
                (toolCall): toolCall is MessageContentToolUse =>
                    toolCall !== null,
            );
