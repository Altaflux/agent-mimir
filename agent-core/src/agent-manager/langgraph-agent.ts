...
import {
  ...
  extractAllTextFromComplexResponse,
} from "../utils/format.js";
import {
  commandContentToBaseMessage,
...
} from "../utils/instruction-mapper.js";
import z from "zod";
...
export type AgentGraphType = CompiledStateGraph<
...
export function materializeAgentInput(
  input: AgentInput,
): MaterializedAgentInput {
...
function buildNotificationProcessingMessage(
  ...
  const header = [
    `Plugin: ${notification.pluginPrefix ? `${notification.pluginPrefix} / ` : ""}${notification.pluginId}`,
...
export class LanggraphAgent implements Agent {
  ...
  async *call(args: {
    ...
    IntermediateAgentMessage,
    { message: AgentResponse; checkpointId: string },
    unknown
  > {
    let stateConfig = {
    ...
    if (state.next.length > 0 && state.next[0] === "human_review_node") {
      ...
      if (args.input) {
        ...
        graphInput = new Command({
          resume: {
            type: "response",
            args: extractAllTextFromComplexResponse(message.content),
      ...
      const materializedInput = args.input
        ? materializeAgentInput(args.input)
        : null;
            ...
            messages: [
              new HumanMessage({
                ...
                content: complexResponseToLangchainMessageContent(
                  materializedInput.message.content,
                ),
    ...
    let generator = this.executeGraph(
      graphInput,
      args.sessionId,
      args.checkpointId,
      args.abortSignal,
    ...
    const newState = await this.graph.getState({
      ...stateConfig,
      configurable: { thread_id: args.sessionId },
    });
    return {
  ...
  > {
    ...
    let commandHandler = this.args.commands.find(
      (ac) => ac.name == args.command.name,
    )!;
    let newMessages = await commandHandler.commandHandler(
  ...
  async shutDown() {
    await Promise.all(
      this.args.plugins.map(async (plugin) => await plugin.destroy()),
    );
  }

  async reset(args: { sessionId: string }): Promise<void> {
    await Promise.all(
      this.args.plugins.map(async (plugin) => await plugin.reset()),
    );
    await this.args.workspace.reset();

    const checkpointer = this.graph.checkpointer as
      | BaseCheckpointSaver
      | boolean
      | undefined;
    if (checkpointer && checkpointer !== true) {
      await checkpointer.deleteThread(args.sessionId);
    }
  }
  ...
  async *executeGraph(
    ...
    checkpointId?: string,
    abortSignal?: AbortSignal,
  ): AsyncGenerator<IntermediateAgentMessage, AgentResponse, unknown> {
    let stateConfig = {
      streamMode: [
        "messages" as const,
        "values" as const,
    ...
    const getMessageChunkToStream = async (
      chunkText: string,
    ): Promise<{
      text: string;
      responseAttributes: Record<string, any> | undefined;
    }> => {
      if (!chunkText) {
      ...
      if (!responseAttributes) {
        responseAttributes =
          await this.args.fieldMapper.readInstructionsFromResponseString(
            streamMarkerBuffer,
          );
      }
      const textAfterMarker = streamMarkerBuffer
        .slice(markerIndex + USER_RESPONSE_MARKER.length)
        .trimStart();
      streamMarkerBuffer = "";
    ...
    while (true) {
      let stream = await this.graph.stream(graphInput, {
        ...stateConfig,
        signal: abortSignal,
        configurable: {
          thread_id: sessionId,
      ...
      for await (const state of stream) {
        if (state[0] === "messages") {
          ...
          if (baseMessage.type === "ai") {
            const textToStream = await getMessageChunkToStream(
              baseMessage.text,
            );
            if (!textToStream) {
              continue;
          ...
          if ((messageState.messages?.length ?? 0) > 0) {
            ...
            if (
              ...
              lastMessage.id !== lastKnownMessage?.id
            ) {
              ...
              yield {
                ...
                toolResponse: {
                  ...
                  response: lCmessageContentToContent(
      ...
      const lastMessage = state.values.messages[
        state.values.messages.length - 1
      ] as BaseMessage;
      const responseAttributes: Record<string, any> =
        state.values["responseAttributes"];
      if (
        state.tasks.length > 0 &&
        state.tasks[0].name === "human_review_node"
      ) {
      ...
      return {
        ...
        output: {
          ...
          sharedFiles:
            (lastMessage.additional_kwargs["shared_files"] as SharedFile[]) ??
            [],
        },
        responseAttributes: responseAttributes,
...
function materializeInterruptedToolCalls(
  ...
  if (Array.isArray(args)) {
    const toolCalls = args
      ...
      .filter(
        (toolCall): toolCall is MessageContentToolUse => toolCall !== null,
      );
...
function materializeToolCallFromUnknown(
  value: unknown,
): MessageContentToolUse | null {
