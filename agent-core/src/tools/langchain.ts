...
export class LangchainToolToMimirTool<
  SchemaT = ToolInputSchemaBase,
> extends AgentTool<SchemaT> {
  schema: SchemaT;
  name: string;
  description: string;
  ...
  toMessageContent(response: any) {
    if (
      this.tool.responseFormat === "content_and_artifact" &&
      Array.isArray(response)
    ) {
      const responseContent = response[0];
      return lCmessageContentToContent(
        Array.isArray(responseContent)
          ? (responseContent as MessageContent)
          : [responseContent],
      );
    }
    return lCmessageContentToContent(
      Array.isArray(response) ? (response as MessageContent) : [response],
    );
  }
}
