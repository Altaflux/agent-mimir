...
export { WebDriverManager, PlaywrightDriverOptions as SeleniumDriverOptions, WebBrowserOptions } from "./driver-manager.js";
...
async function addLabels(buffer: Buffer, coordinates: Map<string, InteractableElement>) {
    ...
    const collisionPadding = Math.max(1, Math.min(blockWidth, blockHeight) * 0.03);
    const labelColors = ["#0072B2", "#D55E00", "#009E73", "#CC79A7", "#6F4E9A", "#C75600", "#008C8C", "#C43C65"];
    ...
    const overlapsPlacedLabel = (candidate: { x: number, y: number, width: number, height: number }) => {
    ...
    const findLabelY = (labelX: number, preferredY: number) => {
        ...
        for (const candidateY of candidateYs) {
            ...
            const candidate = { x: labelX, y: labelY, width: blockWidth, height: blockHeight };
    ...
    for (const [i, mask] of coordinates.entries()) {
        ...
        const elementTop = clamp(Math.min(mask.location.top, mask.location.bottom), 0, height);
        const elementBottom = clamp(Math.max(mask.location.top, mask.location.bottom), 0, height);
        const accentColor = labelColors[labelIndex % labelColors.length];
        labelIndex++;
        const targetX = clamp(mask.location.left + ((mask.location.right - mask.location.left) / 2), 0, width);
        const targetY = clamp(elementTop + ((elementBottom - elementTop) / 2), 0, height);
        ...
        const preferredLabelY = labelYBelow <= maxLabelY
            ? labelYBelow
            : labelYAbove >= 0
                ? labelYAbove
                : clamp(targetY - (blockHeight / 2), 0, maxLabelY);
        const labelY = findLabelY(labelX, preferredLabelY);
        placedLabels.push({ x: labelX, y: labelY, width: blockWidth, height: blockHeight });
        const arrowStartX = labelFitsOnLeft ? labelX + blockWidth : labelX;
        const arrowStartY = labelY + (blockHeight / 2);
        const leaderDistance = Math.hypot(targetX - arrowStartX, targetY - arrowStartY);
        const leaderSvg = leaderDistance > Math.max(blockWidth, blockHeight) * 0.8
