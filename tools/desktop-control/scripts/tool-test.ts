...
import type { AgentTool, ToolResponse } from "@mimir/agent-core/tools";
...
async function main() {
    ...
    const stepsFile = options.steps ? await loadStepsFile(options.steps) : undefined;
    ...
    if (options.pixelWidth) {
        pluginOptions.pixelImageWidth = parsePositiveInteger(options.pixelWidth, "--pixel-width");
...
function readNextValue(argv: string[], index: number, optionName: string): string {
...
function parseMouseMode(mode: string): DesktopControlOptions["mouseMode"] {
    ...
    if (normalized === "PIXEL" || normalized === "COORDINATES" || normalized === "SOM") {
        return normalized;
    }
    throw new Error(`Invalid --mode value: ${mode}. Expected PIXEL, COORDINATES, or SOM.`);
}
...
async function loadStepsFile(filePath: string): Promise<{ mode?: string; pixelImageWidth?: number; steps: ToolTestStep[] }> {
    ...
    if (!parsed || typeof parsed !== "object" || !Array.isArray(parsed.steps)) {
        throw new Error(`Steps file must contain an array of steps or an object with a steps array: ${resolvedPath}`);
    }

    if (parsed.pixelImageWidth !== undefined && (!Number.isInteger(parsed.pixelImageWidth) || parsed.pixelImageWidth <= 0)) {
        throw new Error(`steps.pixelImageWidth must be a positive integer: ${resolvedPath}`);
    }
...
async function runSteps(args: {
    ...
    for (const [index, step] of args.steps.entries()) {
        ...
        if (normalizedStep.action === "prepare") {
            await prepareContext(args.plugin, args.mode, normalizedStep.saveDir ?? args.defaultSaveDir);
        } else if (normalizedStep.action === "tool") {
            await invokeTool(args.tools, normalizedStep.tool, normalizedStep.input ?? {});
...
function normalizeStep(step: ToolTestStep): NormalizedToolTestStep {
    ...
    if ("action" in step) {
        if (step.action === "prepare" || step.action === "tool" || step.action === "wait" || step.action === "list") {
            return step;
        }
        throw new Error(`Unknown step action: ${(step as { action: unknown }).action}`);
    }
...
async function prepareContext(plugin: AgentPlugin, mode: DesktopControlOptions["mouseMode"], saveDir?: string) {
    console.log(`Preparing ${mode} screenshot context...`);
    const additionalContent = await plugin.additionalMessageContent({ content: [] });
...
async function invokeTool(tools: AgentTool[], toolName: string, input: unknown): Promise<ToolResponse> {
...
function printAdditionalContent(items: AdditionalContent[]) {
    ...
    for (const [index, item] of items.entries()) {
        console.log(`Additional content ${index + 1}: saveToChatHistory=${item.saveToChatHistory}, displayOnCurrentMessage=${item.displayOnCurrentMessage}`);
...
function printContent(content: ComplexMessageContent[]) {
    for (const [index, part] of content.entries()) {
        if (part.type === "text") {
            ...
            console.log(`[${index}] image: mimeType=${part.mimeType ?? "unknown"}, base64Bytes=${part.data?.length ?? 0}`);
        }
    }
}

async function saveAdditionalContentImages(items: AdditionalContent[], saveDir: string) {
    ...
    for (const item of items) {
        for (const part of item.content) {
            ...
            const filePath = path.join(resolvedDir, `desktop-context-${imageIndex}.${extension}`);
