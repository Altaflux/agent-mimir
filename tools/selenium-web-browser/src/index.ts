...
const configureDriver = async (options: SeleniumDriverOptions) => {
    let driverName: "chromedriver" | "geckodriver" | "iedriver" | undefined = undefined;
    ...
    switch (options.browserName) {
        ...
        case 'edge': {
            builder = builder.forBrowser("edge")
                .setEdgeOptions(new EdgeOptions().addArguments('--headless'))
            driverName = 'iedriver'
            break;
        }
        default: {
...
const getHtml = async (
    ...
    try {
        //   await update(driverConfiguration.driverManagerOpts);
        //  await start(driverConfiguration.driverManagerOpts);
        driver = driverConfiguration.driverBuilder
            .build();
        ...
        try {
            // await shutdown(driverConfiguration.driverManagerOpts);
        } catch (e) {
            //
        }
    }
