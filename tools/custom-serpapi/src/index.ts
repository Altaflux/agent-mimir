...
export class SerpAPI extends Tool {
    ...
    async _call(input: string) {
        ...
        return this.collectResponse(res);
    }

    collectResponse(res: any) {
        let result = "";
        if (res.answer_box?.answer) {
            result = `\n${result}Search results :${res.answer_box.answer}\n`;
        }

        if (res.answer_box?.snippet) {
            result = `\n${result}Additional Information: ${res.answer_box.snippet}\n`;
        }

        if (res.answer_box?.snippet_highlighted_words) {
            result = `\n${result}Highlighted information: ${res.answer_box.snippet_highlighted_words[0]}\n`;
        }

        if (res.knowledge_graph?.description) {
            result = `\n${result}General Knowledge: ${res.knowledge_graph.description}\n`;
        }

        if (res.organic_results?.[0]?.snippet) {
            const relevantLinks = res.organic_results.slice(0, 5).map((link: any) => {
                return `- Title: "${link.title}": Link: "${link.link}" Snippet from link: "${link.snippet}"`;
            }).join("\n");
            result = `${result}\nRelevant Links:\n${relevantLinks}\n`;
