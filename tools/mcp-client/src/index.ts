...
export class McpClientPluginFactory implements PluginFactory {
  ...
  async create(_context: PluginContext): Promise<AgentPlugin> {
    try {
      ...
      const combinedResult = this.combineResults(
        clientResults.map((c) => c.pluginResult),
      );
        ...
        (prompt) =>
          ({
            ...
            commandHandler: async (args) => {
              ...
              return ContentConverter.convertPromptMessagesToCommandContent(
                response.messages,
              );
            },
  ...
  private combineResults(results: PluginResult[]): {
    tools: AgentTool[];
    prompts: AgentCommand[];
  } {
...
namespace ContentConverter {
  ...
  function convertContent(content: McpContent): ComplexMessageContent {
    switch (content.type) {
      ...
      default:
        throw new Error(
          `Unsupported content type: ${(content as { type: string }).type}`,
        );
    }
  }

  export function convertEmbeddedResourceToComplexResponse(
  ...
  export function convertToolToToolResponse(
  ...
  export function convertPromptMessagesToCommandContent(
    messages: PromptMessage[],
  ): CommandContent[] {
    const commandContents = messages.map((message) => {
      const role = message.role === "assistant" ? "assistant" : "user";
      let content: ComplexMessageContent = convertContent(
  ...
  function packConsecutiveCommandContents(
    arr: CommandContent[],
  ): CommandContent[] {
    ...
    try {
      ...
      if (!resource || resource.contents.length === 0) {
        return [
          {
            type: "text",
            text: `No content found for resource ${arg.resourceURI}`,
          },
        ];
      }
      ...
      return [
        {
          type: "text",
          text: `Failed to read resource from ${arg.mcpServer}: ${err}`,
        },
      ];
    }
  }
      ...
      await Promise.all(
        this.clients.map(async (c) => {
          ...
          try {
            ...
            const resourceTemplate = resources.resources
              .map((r) => {
