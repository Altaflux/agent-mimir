...
export class CodeInterpreterPluginFactory implements PluginFactory {
  pluginId = "codeInterpreter";
  displayName = "Workspace Code Interpreter";
...
class CodeInterpreterPlugin extends AgentPlugin {
  ...
  async tools() {
    return [new PythonCodeInterpreter(this.workSpace?.workingDirectory)];
  }
...
class PythonCodeInterpreter extends AgentTool {
  ...
  schema = z.object({
    externalDependencies: z
      .array(z.string().describe("Name of a dependency to install."))
      .optional()
      .describe(
        "The list of external dependencies to download and install which are going to be required by the script.",
      ),
...
This interpreter DOES have access to all the files in your workspace which can be accessed thru \"os.getenv('WORKSPACE')\". If you are given the task to create a file or they ask you to save it, then save the files inside the directory who's path is stored in the OS environment variable named "WORKSPACE" accessible like \"os.getenv('WORKSPACE')\". `;
  ...
  protected async _call(arg: z.input<this["schema"]>): Promise<ToolResponse> {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "python-code-interpreter-"),
    );
    ...
    try {
      ...
      const pyenv = await this.executeShellCommand(
        `cd ${tempDir} && python -m venv .`,
      );
      if (pyenv.exitCode !== 0) {
        throw new Error(
          `Failed to create python virtual environment: ${pyenv.output}`,
        );
      }

      await fs
        .appendFile(
          path.join(tempDir, scriptsDir, "activate.bat"),
          `\nSET WORKSPACE=${this.workDirectory}\n`,
        )
        .catch(() => {
          console.warn("Failed to write to activate.bat");
        });
      await fs
        .appendFile(
          path.join(tempDir, scriptsDir, "activate"),
          `\nexport WORKSPACE=${this.workDirectory}\n`,
        )
        .catch(() => {
          console.warn("Failed to write to activate");
        });
      await fs
        .appendFile(
          path.join(tempDir, scriptsDir, "Activate.ps1"),
          `\n$Env:WORKSPACE = "${this.workDirectory}"\n`,
        )
        .catch(() => {
          console.warn("Failed to write to Activate.ps1");
        });

      const activeScriptCall =
        process.platform === "win32" ? `activate` : `. ./activate`;

      const beforeExecutionOutputFileList = this.workDirectory
        ? await fs.readdir(this.workDirectory)
        : [];
      ...
      for (const libraryName of arg?.externalDependencies ?? []) {
        ...
        const installationResult = await this.executeShellCommand(
          `cd ${path.join(tempDir, scriptsDir)} && ${activeScriptCall} && python -m pip install ${libraryName}`,
        );
        if (installationResult.exitCode !== 0) {
          libraryInstallationResult = {
            exitCode: installationResult.exitCode,
            output:
              installationResult.output +
              "\n------\n" +
      ...
      const result = await this.executeShellCommand(
        `cd ${path.join(tempDir, scriptsDir)} && ${activeScriptCall} && cd ${this.workDirectory} && python ${scriptPath}`,
      );

      let fileList = "";
      if (this.workDirectory) {
        const files = (await fs.readdir(this.workDirectory)).filter(
          (item) => !beforeExecutionOutputFileList.includes(item),
        );

        fileList =
          files.length === 0
            ? ""
            : `The following files were created in the workspace: ${files.map((fileName: string) => `"${fileName}"`).join(" ")}`;
      }
      const dependenciesWarning =
        libraryInstallationResult?.exitCode !== 0
  ...
  async executeShellCommand(command: string) {
    return await new Promise<{
      ...
      ls.on("close", (code) => {
        const finalOutput = this.workDirectory
          ? output.replaceAll(this.workDirectory, "./WORKSPACE")
          : output;
        resolve({
          exitCode: code ?? 0,
          output: finalOutput,
