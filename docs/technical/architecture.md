...
- `agent-core`: reusable core framework
- `cli-interface`: CLI runtime
- `discord-interface`: Discord runtime
...
## Core domains in `agent-core`

### 1) Agent contract and state

`agent-core/src/agent-manager/index.ts` defines:
...
`agent-core/src/agent-manager/langgraph-agent.ts` provides the concrete `LanggraphAgent` wrapper around a compiled LangGraph graph.

### 2) Agent implementations

There are two main agent implementations:

- Tool agent (`agent-core/src/agent-manager/tool-agent/`)
  - Uses LangChain tool calling (`tool_calls`)
  - Executes tools in the graph tool node
- Code agent (`agent-core/src/agent-manager/code-agent/`)
...
`agent-core/src/plugins/index.ts` defines extension points:
...
`agent-core/src/schema.ts` defines `ComplexMessageContent`:
