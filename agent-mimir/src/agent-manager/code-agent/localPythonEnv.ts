...
export async function localPythonEnvironment(port: number, tools: string[], code: string, toolInit: () => void): Promise<string> {

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mimir-code-interpreter-'));

    const scriptPath = path.join(tempDir, 'script.py');
    const pythonScript = getPythonScript(port, tools, code);
